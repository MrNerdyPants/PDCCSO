...
object phver1 extends Serializable {
...
def mean(arr:List[Double]):Double={    
   ...
   //maping fitnesses of all crows 
    ...
    var population=(x,List.range(0,Pop)).zipped.map((x_p,i)=>new com.dust.SCSA.HybridCrow(x_p, Pop, dim, min, max,fitnessesC(i),2.12))
    ...
    var para:RDD[HybridCrow]=sc.parallelize(population, numSlices).persist()
    ...
    while(t<max_it){
     ...
     /* 
      if(cflag==true){
        ...
        val rempop=(remx,List.range(0,rem)).zipped.map((x_p,i)=>new com.dust.SCSA.HybridCrow(x_p, Pop, dim, min, max,remf(i)))
    ...
    while (i<partitionsize){
      ...
      var i_kDiff=crows(i).sumf(crows(i).f,sortedpopulation.map(_.fbest).toArray ,0,sortedpopulation.length)
      ...
      global=non_Neigh(0)
	    ...
	    if (crows(local).fbest<crows(global).fbest){
	      //call NLS  
	      crows(i).NLS(crows(i).x ,crows(local).y)
	    }
	    else{
	      //call NGS
	      crows(i).NGS(crows(i).x,crows(global).y)
	    }
      ...
      */
      ...
      val probability =math.exp((-(math.abs(crows(i).fbest-crows(i).f)))/crows(i).fl)
      //condition for PRC
   ...
   /* //replace the population at random
    ...
    top2crow.update(topcrow)
      ...
      println("flight length ",sortedpopulation(0).fl)
