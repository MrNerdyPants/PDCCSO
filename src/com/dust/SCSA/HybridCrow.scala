...
class HybridCrow(var crow:Array[Double],var pop: Int, var Dim:Int, var min:Double, var max:Double ,var fitnesses :Double, flight:Double)extends Serializable {
  ...
  def NLS(x_i:Array[Double],m_local:Array[Double])={
  ...
  def NGS(x_i:Array[Double],mem_global: Array[Double])={
  ...
  def WAS(x_i:Array[Double],x_r:Array[Double],mem_global:Array[Double])={
  ...
  def flight(max_it:Int,t:Int)={
    if(f<=fbest){
      fl=( 2.02 -t *((1.08)/ max_it))
    }
  }
