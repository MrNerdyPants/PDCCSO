...
object npver3 extends Serializable {
...
def mean(arr:List[Double]):Double={    
   ...
   //maping fitnesses of all crows 
    ...
    val perc=((((population.length).toDouble/(100).toDouble).toDouble*20)/numSlices).toInt
   ...
   /* //replace the population at random
    ...
    var selected=/*crows.sortWith(_.fbest<_.fbest)*/topchk.take((((20.toDouble*(crows.length).toDouble/(100).toDouble))).toInt) //.take((crows.length/100)*20)
