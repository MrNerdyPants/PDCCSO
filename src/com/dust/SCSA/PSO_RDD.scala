...
object PSO_RDD extends Serializable{
  ...
  def main(args:Array[String]): Unit ={
    ...
    val conf=new SparkConf().setAppName("2W4C3G :: "+dimension+"-"+no_of_particles+"-"+no_of_iteration_External+"-"+no_of_iteration_Internal)
                            ...
                            .set("spark.eventLog.dir","file:///home/nerdypants/Downloads/spark-3.0.1-bin-hadoop2.7/")
    ...
    var swarm_rdd1 = sc.parallelize(swarm,8)  //No. of partitions, we usually keep it as (no of worker*no of core)

    var swarm_rdd=swarm_rdd1.mapPartitionsWithIndex{
      (idx,iterator)=>
        var swarm=iterator.toArray
        swarm.foreach(f => init_pbest(f))      //Initialize particles p_best
        swarm.foreach(f => f.compute_pbest_fitness()) //compute p_best fitness value
        swarm.foreach(f => f.compute_pfitness())  //compute current fitness value
        swarm.foreach(f => global_best_position(f))  //compute global best position
        var best=swarm
        best.toIterator
    }
    ...
    */
    ...
    var transformedrdd=swarm_rdd.mapPartitionsWithIndex{
      (idx,iterator)=>
        var particle=iterator.toArray
    ...
    println("first print ",obj_func(g_best))
    for(iteration <- 0 to no_of_iteration_External){
      ...
      updated_velocity_swarm/*temp_rdd*/.collect().foreach(f => global_best_position(f))  //compute new global best
      println("second in loop",iteration," print ",obj_func(g_best))
    ...
    def update_particle(p:Particle):Particle={
     ...
     for(iter <- 0 to no_of_iteration_Internal){        //update the particle
      ...
      //println("While update \nIteration Number : "+iter+" || Particle Number :  "+p.p_id+" || Pbest : "+obj_func(p.p_best)+" || Gbest : "+obj_func(gbest_position))
      }
  ...
  def obj_func(x:Array[Double]):Double = {   //Sphere Function
      var temp:Double  =0
      var casef="1"
      temp=com.dust.SCSA.testfunction.matchTest( x,casef)
